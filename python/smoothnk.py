...
def brdf_fix(data,mindoy=None,maxdoy=None,time_dim=0):
  '''
  The input data dictionary should contain:
  ...
  These can be numpy arrays or masked arrays.

    bands : waveband keys in a dict (e.g. 
            bands = {645.0:6,858.5:7,469.0:8,555.0:9,1240.0:10,1640.0:11,2130.0:12}
            or just an array (e.g. [645.0,858.5]). 
    doy   : day of year
    
    645.0 : etc. reflectance data
   
  '''
  # sort the band keys
  if type(data['bands']) == dict:
    band_keys = np.sort(np.array(data['bands'].keys()))
  else:
    band_keys = np.sort(np.array(data['bands']))
  ...
  refl = []
  for k in band_keys:
    refl.append(ma.array(data[k]))
  refl = ma.array(v)
 
  data['doy'] = np.array(data['doy'])
  # sort the data in equal time 
  mindoy = mindoy or data['doy'].min() 
  maxdoy = maxdoy or data['doy'].max()

  # full dataset:
  args = np.array([slice(0,refl.shape[i]) for i in refl.ndim])
  args[time_dim] = slice(mindoy,maxdoy+1)
...
def smoothn(y,nS0=10,axis=None,smoothOrder=2.0,sd=None,verbose=False,\
  ...
  N = float(sum(array(sizy) != 1)); # tensor rank of the y-array
  hMin = 1e-6; hMax = 0.99;
  # (h/n)**2 = (1 + a)/( 2 a)
  # a = 1/(2 (h/n)**2 -1) 
  # where a = sqrt(1 + 16 s)
  ...
  while RobustIterativeProcess:
    ...
    while tol>TolZ and nit<MaxIter:
        ...
        if isauto and not remainder(log2(nit),1):
          ...
          if not s0:
            pspan = np.arange(nS0)*(1./(nS0-1.))*(log10(sMaxBnd)-log10(sMinBnd)) + log10(sMinBnd)
            g = np.zeros_like(pspan)
            sspan = 10**pspan
            for i,p in enumerate(pspan):
              g[i] = gcv(p,Lambda,aow,DCTy,IsFinite,Wtot,y,nof,noe,smoothOrder)
              #print s,g[i]
            xpost = [np.median(sspan[g==g.min()])]
            if verbose:
              print '==============='
              print nit,tol,g.min(),xpost[0]
              print '==============='
          else:
            xpost = [s0]
            #p = log10(s0)
            #pspan = np.array([p/1.2,p,p*1.2])
            #g = np.zeros_like(pspan)
            #sspan = 10**pspan
            #for i,p in enumerate(pspan):
            #  g[i] = gcv(p,Lambda,aow,DCTy,IsFinite,Wtot,y,nof,noe,smoothOrder)
